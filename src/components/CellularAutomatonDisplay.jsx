...
const CellularAutomatonDisplay = (props) => {
    ...
    const run = useCallback(() => {
        if (status.current.isRunning) {
            if (
                generationIndexRef.current < execOutputData.generations.length
            ) {
        ...
        <>
            ...
            >
                {execOutputData !== null ? (
                    ...
                    >
                        {execOutputData.generations[generationIndex].map(
                            (row, i) =>
                                row.map((cellState, j) => (
                                    <GameOfLifeCell
                                        key={`(${i},${j})`}
                    ...
                    <Typography ml={1} variant="body1" color="initial">
                        Hit the plus button in order to create a new enviroment
                        or load the cellular automaton generations file in order
                        to create the view.
                    </Typography>
                )}
                {hideFloatingButton ? (
